...
DB.prototype.getConnectedPeers = function(uuid, passphrase, resolve, reject) {
    runStatementIfValidDevice(uuid, passphrase, function() {
        const peersStatement = 'MATCH (device:Device {uuid: {uuid}}) MATCH (d:Device)-[:SEES*]->(device) RETURN count(d) AS connectedPeers';
        const peersParams = {
            uuid: uuid
        };

        runStatement(peersParams, peersStatement, resolve, reject);
    }, reject);
};
...
DB.prototype.disconnectDevice = function(uuid, resolve, reject) {
    const deviceStatement = 'MATCH (device:Device {uuid: {uuid}}) MATCH (device)-[rel:SEES]->(:Device) WHERE NOT rel IS NULL DELETE rel SET device.updateVersion = 1';
    const deviceParams = {
        uuid: uuid
    };
...
function createDevice(nickname, uuid, passphrase, resolve, reject) {
    const registrationStatement = 'CREATE (device:Device {nickname: {nickname}, uuid: {uuid}, passphrase: {passphrase}})';
    ...
    runStatement(registrationParams, registrationStatement, resolve, reject);
}

function changeNicknameForUUID(uuid, nickname, resolve, reject) {
    const nicknameStatement = 'MATCH (device:Device {uuid: {uuid}}) SET device.nickname = {nickname}';
    const nicknameParams = {
        uuid: uuid,
        nickname: nickname
    };
...
function addDeviceSeeingDevice(uuid, otherDeviceUUID, resolve, reject) {
    const deviceStatement = 'MATCH (device:Device {uuid: {uuid}}) MATCH (otherDevice:Device {uuid: {otherDeviceUUID}}) CREATE (device)-[rel:SEES]->(otherDevice)';
    const deviceParams = {
        uuid: uuid,
...
function removeDeviceSeeingDevice(uuid, otherDeviceUUID, resolve, reject) {
    const deviceStatement = 'MATCH (device:Device {uuid: {uuid}}) MATCH (otherDevice:Device {uuid: {otherDeviceUUID}}) MATCH (device)-[rel:SEES]->(otherDevice) DELETE rel';
    const deviceParams = {
        uuid: uuid,
        otherDeviceUUID: otherDeviceUUID
    };
...
function runStatementIfValidDevice(uuid, passphrase, resolve, reject) {
    getDeviceIfExists(uuid, function(result) {
        if (result['device.passphrase'] == passphrase) {
            resolve();
        } else {
